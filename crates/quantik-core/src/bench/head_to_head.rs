...
/// Identity of one head-to-head game (for checkpoint resume): mover,
/// responder, position, seed.
pub type GameKey = (String, String, String, u64);

pub fn record_key(record: &Value) -> GameKey {
    (
...
pub fn run_head_to_head(
    ...
    for position in positions {
        let bb = State::from_qfen(position["qfen"].as_str().unwrap_or_default())?.bb;
        ...
        for &seed in seeds {
            for (mover, responder) in [(adapter_a, adapter_b), (adapter_b, adapter_a)] {
                ...
                let (winner, plies) = play_game(mover, responder, &bb, seed)?;
                let record = json!({
