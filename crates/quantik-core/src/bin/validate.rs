...
use quantik_core::constants::{VERSION, FLAG_CANON};
...
fn main() {
    let fixtures: Vec<(&str, &str, [u16; 8], bool, usize, &str)> = vec![
        ("empty", "..../..../..../....", [0,0,0,0,0,0,0,0], false, 64,
         "010200000000000000000000000000000000"),
        ("single_corner", "A.../..../..../....", [1,0,0,0,0,0,0,0], false, 53,
         "010200000000000000100000000000000000"),
        ("alternating", "Ab../..../..../....", [1,0,0,0,0,2,0,0], false, 50,
         "010200000000000000100000000000010000"),
        ("winning_row", "AbCd/..../..../....", [1,0,4,0,0,2,0,8], true, 40,
         "010200000000000101000010100000000000"),
        ("winning_col", "A.../B.../C.../D...", [1,16,256,4096,0,0,0,0], true, 0,
         "010200010010010010000000000000000000"),
        ("midgame", "Ab../c.D./..../...A", [32769,0,0,64,0,2,16,0], false, 28,
         "010200000000000201800008004000000000"),
        ("full", "AbcD/bCdA/cDaB/DaBc", [129,18432,32,4616,9216,18,33028,64], true, 0,
    ...
    for (idx, (name, qfen, planes, exp_win, exp_moves, exp_ck_hex)) in fixtures.iter().enumerate() {
        ...
        for _ in 0..iters { let _ = SymmetryHandler::find_canonical(&bb); }
        let canon_ns = start.elapsed().as_nanos() as f64 / iters as f64;

        let start = Instant::now();
        for _ in 0..iters { let _ = generate_legal_moves(&bb); }
        let moves_ns = start.elapsed().as_nanos() as f64 / iters as f64;

        let start = Instant::now();
        for _ in 0..100_000u64 { let _ = has_winning_line(&bb); }
